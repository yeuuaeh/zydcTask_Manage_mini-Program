<template>
	<view class="content">
		<view v-for="(item, index) in listArray" :key="index" class="listItem">
			...
			<view class="nameType">{{ item.incomeMXDescription }}</view>
			...
			async getIntegralListRecordAjax(userids) {
				...
				if (res.data.code == 0) {
					...
					res.data.data.array.forEach(item => {
						let id = item.tradeType;
						if (id == "income") {
							let incomeType = item.incomeType;
							// console.log("start:===",incomeType)
							let statusType = item.status;
							console.log("statusType:===", statusType)
							item.incomeMXDescription = _self.incomeMX[incomeType] || ''; // 添加新属性保存描述
